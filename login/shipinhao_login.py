...
from config import SHIPINHAO_STATE_FILE, BROWSER_CHANNEL, STATE_DIR

logger = setup_logger("shipinhao_login")
...
async def login_and_save_state():
    ...
    async with async_playwright() as p:
        ...
        try:
            await context.storage_state(path=SHIPINHAO_STATE_FILE)
            logger.info(f"状态已保存到 {SHIPINHAO_STATE_FILE}")
